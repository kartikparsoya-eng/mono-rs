...
function emitChain(ast: AST, depth: number, lines: string[]): void {
  if (ast.where) {
    ...
    if (w.type === 'simple') {
      ...
      w.type === 'correlatedSubquery' &&
      w.op === 'EXISTS' &&
      !w.flip &&
      !w.scalar
    ) {
      lines.push(
        `${pad(depth)}.whereExists('${relNameFromCsq(w.related)}'${maybeSubqCallback(w.related, depth)})`,
      );
  ...
  if (ast.start) {
    lines.push(
      `${pad(depth)}.start(${JSON.stringify(ast.start.row)}, {inclusive: ${!ast.start.exclusive}})`,
    );
  }
...
function renderCondition(c: Condition, depth: number): string {
  ...
  if (c.type === 'correlatedSubquery') {
    ...
    const cb = hasInterestingSub(sub)
      ? `, q => q.where(({cmp, or, and, exists, not}) => ${renderCondition(sub.where!, depth + 1)})`
      : '';
