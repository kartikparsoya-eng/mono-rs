...
export const patterns = {
  ...
  p07_where_is_null: () => zql.messages.where('visibleTo', 'IS', null),
  ...
  p09_where_ilike: () => zql.messages.where('body', 'ILIKE', '%standup%'),
  ...
  p24_flip_simple_filtered: () =>
    zql.messages.where(({exists}) =>
      exists('conversation', c => c.where('channelId', 'ch-pub-1'), {
        flip: true,
      } as const),
    ),
  ...
  p25_flip_multi_parent: () =>
    zql.attachments.where(({exists}) =>
      exists('message', m => m.where('authorId', 'u1'), {flip: true} as const),
    ),
  ...
  // 2 leaves 8 rows.
  p28_not_in: () => zql.messages.where('id', 'NOT IN', ['m-1', 'm-2']),
  ...
  p35_spread_or: () =>
    zql.messages.where(({or, cmp}) =>
      or(...['m-1', 'm-3', 'm-5'].map(id => cmp('id', '=', id))),
    ),
