  ...
  if (op === 'IN' || op === 'NOT IN') {
    if (col.type === 'number' || col.type === 'number?') {
      return [
        [],
        [NUMBER_LITERALS[0]],
        [NUMBER_LITERALS[0], NUMBER_LITERALS[1]],
      ];
    }
...
function simpleConditions(table: TableDef): SimpleCondition[] {
  ...
  return [
    ...roundRobin(perCol.map(c => c.primary)),
    ...roundRobin(perCol.map(c => c.secondary)),
  ];
}
      ...
      for (const flip of [undefined, true, false] as const) {
        for (const scalar of [undefined, true] as const) {
          ...
          out.push({
            ...
            related: {
              correlation: {
                parentField: cKey(rel.parentField),
                childField: cKey(rel.childField),
              },
 ...
 * fuzzer output.
 */
function csqWithNestedCsqSubWhere(
  table: TableDef,
  ctx: GenCtx,
): CorrelatedSubqueryCondition[] {
  ...
  for (const rel of table.relationships) {
    ...
    const grandLeaf =
      simpleConditions(grandTable).find(l => l.op === '=') ??
      simpleConditions(grandTable)[0];
    if (!grandLeaf) continue;
    const innerCsq: CorrelatedSubqueryCondition = {
      type: 'correlatedSubquery',
      related: {
        correlation: {
          parentField: cKey(grandRel.parentField),
          childField: cKey(grandRel.childField),
        },
        subquery: {
          table: grandRel.childTable,
          alias: nextAlias(grandRel.name),
          where: grandLeaf,
        },
      },
      op: 'EXISTS',
...
function whereShapes(table: TableDef, ctx: GenCtx): Condition[] {
  ...
  const leafPool: Condition[] = [...simples.slice(0, 4), ...csqs.slice(0, 6)];
...
function conditionNodes(c: Condition): number {
  ...
  if (c.type === 'correlatedSubquery') {
    return (
      1 +
      (c.related.subquery.where ? conditionNodes(c.related.subquery.where) : 0)
    );
  }
  let n = 1;
...
function relatedShapes(
  ...
  const rels = table.relationships.filter(
    r => !ctx.ancestorTables.has(r.childTable),
  );
  ...
  for (const rel of rels) {
    ...
    const childOther = childTable.columns.find(
      c => !childTable.pk.includes(c.name),
    )?.name;
    if (!childPk) continue;
  ...
  if (MAX_RELATED_DEPTH >= 2) {
    for (const rel of rels) {
      ...
      const childTable = SCHEMA[rel.childTable];
      if (!childTable) continue;
      const grand = childTable.relationships.find(
        r => !childCtx.ancestorTables.has(r.childTable),
      );
      if (!grand) continue;
...
function topOptionsForTable(table: TableDef): TopOption[] {
  ...
  if (orderBy2) {
    ...
    out.push({
      limit: 5,
      orderBy: orderBy2,
      start: {row: sampleRow(table, orderBy2), exclusive: false},
    });
    out.push({
      limit: 5,
      orderBy: orderBy2,
      start: {row: sampleRow(table, orderBy2), exclusive: true},
    });
  }
  return out;
}
...
function sampleRow(
  ...
  for (const [col] of ordering) {
    ...
    row[col] =
      def?.type === 'number' || def?.type === 'number?'
        ? NUMBER_LITERALS[1]
        : STRING_LITERALS[0];
  }
  return row;
...
function generateForTable(table: TableDef): AST[] {
  ...
  const interestingWheres = wheres
    .filter(
      w =>
        w.type === 'or' || w.type === 'and' || w.type === 'correlatedSubquery',
    )
...
async function main(): Promise<void> {
  ...
  for (const ast of deduped) {
    ...
    if (n > MAX_NODES) {
      throw new Error(
        `AST exceeds MAX_NODES=${MAX_NODES} (got ${n}): ${JSON.stringify(ast)}`,
      );
    }
  }
  ...
  const summary = {
    ...
    perTable: Object.fromEntries(
      Object.keys(SCHEMA).map(t => [
        t,
        corpus.filter(c => c.ast.table === t).length,
      ]),
    ),
    byShape: {
      ...
      csqWhere: corpus.filter(c => c.ast.where?.type === 'correlatedSubquery')
        .length,
      ...
      flipTrue: corpus.filter(c =>
        JSON.stringify(c.ast).includes('"flip":true'),
      ).length,
      flipFalse: corpus.filter(c =>
        JSON.stringify(c.ast).includes('"flip":false'),
      ).length,
      scalarTrue: corpus.filter(c =>
        JSON.stringify(c.ast).includes('"scalar":true'),
      ).length,
      notExists: corpus.filter(c =>
        JSON.stringify(c.ast).includes('"NOT EXISTS"'),
      ).length,
    },
    caps: {
      MAX_WHERE_DEPTH,
