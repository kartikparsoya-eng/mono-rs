...
function main(): void {
  ...
  const cov = tryRead<{summary: any; results: CovResult[]; elapsedMs?: number}>(
    covPath,
  );
  const adv = tryRead<{
    summary: any;
  ...
  if (cov) {
    lines.push(
      `- Hydration sweep: \`${covPath}\` — ${cov.results.length} ASTs, ${((cov.elapsedMs ?? 0) / 1000).toFixed(1)}s`,
    );
  } else {
    lines.push(
      `- Hydration sweep: NOT RUN (\`coverage_run.json\` missing — run \`npx tsx harness-coverage.ts\`)`,
    );
  }
  if (adv) {
    lines.push(
      `- Advancement sweep: \`${advPath}\` — ${adv.results.length} ASTs, ${((adv.elapsedMs ?? 0) / 1000).toFixed(1)}s`,
    );
    if (adv.mutations) {
    ...
    lines.push(
      `- Advancement sweep: NOT RUN (\`advance_coverage_run.json\` missing — run \`npx tsx harness-advance-coverage.ts\`)`,
    );
  }
  lines.push('');
  ...
  if (cov && adv) {
    ...
    const aParity =
      as_.total > 0 ? ((as_.ok / as_.total) * 100).toFixed(1) : '–';
    lines.push(`| Total ASTs | ${cs.total} | ${as_.total} |`);
    lines.push(`| OK | ${cs.ok} (${cParity}%) | ${as_.ok} (${aParity}%) |`);
    lines.push(
      `| Diverge | ${cs.diverge} | hyd=${as_.hydrateDiverge}, adv=${as_.advanceDiverge} |`,
    );
    ...
    lines.push(
      `| Diverge | – | hyd=${as_.hydrateDiverge}, adv=${as_.advanceDiverge} |`,
    );
  ...
  if (cov) {
    ...
    if (divs.length > 0) {
      ...
      lines.push('');
      lines.push(
        `${divs.length} AST(s) hydrate to different result sets between TS and RS.`,
      );
      lines.push('');
      ...
      for (const [k, list] of [...grouped.entries()].sort(
        ...
        lines.push(
          `#### \`${k}\` — ${sample.id} (ts=${o.rowCounts.ts}, rs=${o.rowCounts.rs})`,
        );
        lines.push('```ts');
      ...
      lines.push('');
      lines.push(
        '**None.** TS and RS hydrate every corpus AST to identical row sets.',
      );
      lines.push('');
    }
  }
  ...
  if (adv) {
    const advDivs = adv.results.filter(
      r => r.outcome.status === 'advance-diverge',
    );
    const hydDivs = adv.results.filter(
    ...
    if (advDivs.length > 0) {
      ...
      lines.push('');
      lines.push(
        `${advDivs.length} AST(s) hydrate identically but produce different row deltas after the mutation block.`,
      );
      lines.push('');
      ...
      for (const [k, list] of [...grouped.entries()].sort(
        ...
        const sample = list[0];
        const o = sample.outcome as Extract<
          AdvOutcome,
      ...
      lines.push('');
      lines.push(
        '**None.** TS and RS produce identical row deltas after every mutation.',
      );
      lines.push('');
    }
    ...
    if (hydDivs.length > 0) {
      ...
      lines.push('');
      lines.push(
        `${hydDivs.length} AST(s) hydrate to different sets — these are the same bugs as the hydration sweep but observed in the advancement run too.`,
      );
      lines.push('');
    }
  }
  ...
  lines.push(
    'npx tsx harness-coverage.ts               # hydration sweep (~5s)',
  );
  lines.push(
    'npx tsx harness-advance-coverage.ts       # advancement sweep (~2 min)',
  );
  lines.push(
