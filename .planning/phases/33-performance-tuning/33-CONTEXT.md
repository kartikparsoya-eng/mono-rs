...
- **D-05:** The TS oracle path is invoked via `runTsExpensive` callback so the cost (running TS IVM in parallel) is paid lazily — only when sampling fires. **The TS oracle does NOT yet exist as a callable shim in mono-rs** — `pipeline-driver.ts` was rewritten to call only Rust napi during the v4.0 port. To build it, **lift `#advance` (lines 701-810+) and `*addQuery` (line 450+) from the upstream reference repo at `~/Documents/xy-repo/mono/packages/zero-cache/src/services/view-syncer/pipeline-driver.ts`** (verified to exist, ~1139 lines total). Adapt to mono-rs imports + share the snapshotter with the Rust path so both see the same data. Place the lifted code in a sibling file (e.g., `pipeline-driver-ts-oracle.ts`) to keep the Rust path's pipeline-driver.ts diff focused on the parity-check shim. **Realistic effort: ~1-1.5 days, not 2 hours** — much of the lifting is mechanical but adapting the snapshotter share point requires care.
...
### HARDEN-02: OrExists test categories (target ≥22 tests to match Exists, currently 5)

> **Test count correction (verified by `cargo test --release -p zero-ivm-rs --lib -- --list`):** The original CONTEXT.md estimate said exists_op.rs had 44 tests and or_exists had 10. Actual counts: **exists_op.rs has 22 tests, or_exists_op.rs has 5**. The 4.4× gap claim still holds (5 → 22), just at smaller absolute numbers. Target: parity with exists_op.rs at **≥22 tests** in or_exists_op.rs (was ≥30 — revised down to match actual).

- **D-09:** Mirror the 22 existing exists_op.rs tests in or_exists_op.rs. Categories to cover (counts based on exists_op.rs structure verified by --list):
